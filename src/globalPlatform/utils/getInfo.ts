...
const CPLC_FIELDS: [TCPLCKey, number][] = [
    ...
    ['iccManufacturer', 2],
    ['icEmbeddingDate', 2],
    ['icPrePersonalizerID', 2],
    ['icPrePersonalizationDate', 2],
...
export function getInfo(
    card: ICard,
    callback: (err: any, info: IGPInfo) => void,
): void;
...
export function getInfoFromAid(
    ...
    Logger.trace(
        `Getting GP info from ${!importedAid.byteLength ? 'default applet' : `applet "${hexEncode(importedAid)}"`} ...`,
    );
    if (typeof callback === 'undefined') {
    ...
    card.issueCommand(isoSelect(aid))
        .then((selectResponse) => {
            ...
            try {
                ...
                return Promise.reject(
                    new Error(
                        `Error parsing select response: ${error.message}; Data:[${hexEncode(selectResponse.data)}]`,
                    ),
                );
            }
            ...
            if (
                tagSearchResult.length === 1 &&
                tagSearchResult[0].isPrimitive()
            ) {
            ...
            if (
                ...
                cardInfoResult.maxCmdDataLen = new Uint32Array(
                    hexDecode(
                        hexEncode(tagSearchResult[0].value).padStart(8, '0'),
                    ).reverse().buffer,
                )[0];
                Logger.debug(
                    `Max CMD data len: ${cardInfoResult.maxCmdDataLen} bytes`,
                );
            }
                ...
                card.issueCommand(gpGetData(0x00, 0x66))
                    .then((getDataResponse) => {
                        ...
                        if (responseBer.search('/66/73/06').length !== 1) {
                            const errMsg =
                                'Missing mandatory globalPlatform OID tag (73/06).';
                            Logger.warn(errMsg);
                            return resolve();
                        }

                        (cardRecData[0].value as BerObject[]).forEach(
                            (cardRecDataElem) => {
                                // console.log(`reading tag "${cardRecDataTag.tag.hex}"`);
                                if (cardRecDataElem.tag.hex === '06') {
                                    // 1.2.840.114283.1
                                    const oid = Asn1Utils.decodeOID(
                                        cardRecDataElem.value as Uint8Array,
                                    );
                                    Logger.debug(
                                        `Global Platform OID: [${oid.join('.')}]`,
                                    );
                                    return;
                                }

                                if (
                                    !cardRecDataElem.isConstructed() ||
                                    cardRecDataElem.value.length < 1
                                ) {
                                    Logger.warn(
                                        `Unexpected structure of tag "${cardRecDataElem.tag.hex}"`,
                                    );
                                    return;
                                }

                                const isInternalValueValid =
                                    cardRecDataElem.value.reduce(
                                        (isValidValue, internalObj) => {
                                ...
                                try {
                                    switch (cardRecDataElem.tag.hex) {
                                        case '60': // 1.2.840.114283.2.<gpVersion>
                                            ...
                                            decodedOid = Asn1Utils.decodeOID(
                                                cardRecDataElem.value[0]
                                                    .value as Uint8Array,
                                            );
                                            oidStr = decodedOid.join('.');
                                            Logger.debug(
                                                `GP version OID: [${oidStr}]`,
                                            );
                                            if (oidStr.startsWith(oidStrPrefix))
                                                cardInfoResult.gpVersion =
                                                    oidStr.substring(
                                                        oidStrPrefix.length,
                                                    );
                                            break;
                                        ...
                                        case '64': // 1.2.840.114283.4.<scpVersion>.<i>
                                            ...
                                            cardRecDataElem.value.forEach(
                                                (oid) => {
                                                    const scpOid =
                                                        Asn1Utils.decodeOID(
                                                            oid.value as Uint8Array,
                                                        );
                                                    const scpOidStr =
                                                        scpOid.join('.');
                                                    Logger.debug(
                                            ...
                                            cardRecDataElem.value.forEach(
                                                (oidObj) => {
                                                    if (oidObj.tag.hex !== '06')
                                                        return;
                                                    if (
                                                        typeof cardInfoResult.cardAndChipInfo ===
                                                        'undefined'
                                                    )
                                                    ...
                                                    const oidStr =
                                                        Asn1Utils.decodeOID(
                                                            oidObj.value as Uint8Array,
                                                        ).join('.');
                                                    Logger.debug(
                                                        `Card & chip info OID: [${oidStr}]`,
                                                    );
                                                    cardInfoResult.cardAndChipInfo.push(
                                                        oidStr,
                                                    );
                                                },
                                            );
        ...
        .then(() => {
            ...
            return new Promise<void>((resolve) => {
                card.issueCommand(gpGetData(0x00, 0x42))
                    .then((getDataResponse) => {
                        if (
                            hexEncode(getDataResponse.status).toLowerCase() ===
                            '6a88'
                        ) {
                            return resolve();
                        }
                        ...
                        cardInfoResult.iin = hexEncode(
                            iinList[0].value as Uint8Array,
                        );
                        Logger.debug(
                            `Issuer Identification Number(IIN): [${cardInfoResult.iin}]`,
                        );
                        return resolve();
                    })
                    .catch((error) => {
            ...
            return new Promise<void>((resolve) => {
                card.issueCommand(gpGetData(0x00, 0x45))
                    .then((getDataResponse) => {
                        ...
                        cardInfoResult.cin = hexEncode(
                            cinList[0].value as Uint8Array,
                        );
                        Logger.debug(
                            `Card Image Number(CIN): [${cardInfoResult.cin}]`,
                        );
                        return resolve();
                    })
                    .catch((error) => {
                ...
                card.issueCommand(gpGetData(0x9f, 0x7f))
                    .then((getDataResponse) => {
                        ...
                        CPLC_FIELDS.reduce((startOffset, field) => {
                            ...
                            cplcObj[field[0]] = hexEncode(
                                cplcData.subarray(startOffset, endOffset),
                            );
